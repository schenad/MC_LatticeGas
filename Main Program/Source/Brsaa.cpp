...
#include <ctime>
#include <sys/time.h>
double coverage=33.65;
#define A (4e-4)
#define be (0.00225)  //Bond energy
#define steps (10000001)      //Total MC steps
#define trials (1)
#define interval (1)
#define temps (2)
...
int main(){
  ...
  strcat(strcat(strcat(strcat(strcat(num,"-"),firstname),cover),"-"),Tstr);
  ...
  if(s%(steps/10000)==0){
    ...
    cout<<"\r"<<(((double)s/steps+temp+temps*tr)*100.)/trials/temps<<'%'<<"  Time="<<remaintime/3600<<'h'<<(remaintime/60-60*(remaintime/3600))<<"min"<<remaintime%60<<"s ";
    cout<<cout.precision(4)<<fixed<<'\b'<<"dEp="<<(Ef-Ei)*1000<<"meV "<<"T="<<Tf<<"K "<<"T_final="<<T<<"K    ";
    cout.flush();
    gettimeofday(&t_start,NULL);
  }
  ...
  recordE.open(numE,ios::app);
  recordE2.open(numE2,ios::app);
  for(int i=900;i<1000;i++){
    Esum+=Ekp[i];
    Esum2+=Ekp[i]*Ekp[i];
  }
  recordE<<Esum/100.<<endl;
